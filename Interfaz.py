...
def frame_combinador_PDF():
    ...
    for i in range(5):
        ...
        Button_seleccionar_PDF = tk.Button(frame_PDF, text="Seleccionar", command=lambda e=entry_campoentrada_PDF, idx=i: seleccionar_PDF(e, idx, PDF_acumulado))
        Button_seleccionar_PDF.grid(row=i, column=2, padx=10, pady=5)
    
        button_borrar_PDF = tk.Button(frame_PDF, text="X", fg="red", command=lambda e=entry_campoentrada_PDF, idx=i: borrar_PDF(e, idx, PDF_acumulado))
        button_borrar_PDF.grid(row=i, column=3, padx=5, pady=5)
    
    button_guardar_PDF = tk.Button(window, text="Guardar", command=lambda: guardar_PDF_combinado(PDF_acumulado), width=10, height=2, font=("Helvetica", 10))
...
def frame_convertidor_DOC():
    ...
    button_seleccionar_PDF = tk.Button(frame_PDF, text="Seleccionar", command=lambda: seleccionar_PDF(entry_campoentrada_PDF))
    button_seleccionar_PDF.grid(row=0, column=2, padx=10, pady=5)
    
    button_borrar_PDF = tk.Button(frame_PDF, text="X", fg="red", command=lambda: borrar_PDF(entry_campoentrada_PDF))
